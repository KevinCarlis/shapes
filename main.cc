...
shared_ptr<Shape> GetType(vector<Point> &vec) {
	...
	if (vec.empty()) {
		...
		double slope = vec.at(0).slope(*(set.begin()));
		bool line = true;
		for (auto p : set) {
			if (vec.at(0).slope(p) != slope) {
				line = false;
				break;
			}
		}
		if (line) {
			Point origin = vec.at(0);
			Point max = *(set.begin());
			for (auto p: set) {
				if (origin.distance(max) < origin.distance(p))
					swap(max,p);
			}
			shape = make_shared<Line>(origin, max);
		}
		else
		...
		for (size_t i = 2; i < vec.size(); i++) {
			...
			auto loc = find(vertices.begin(), vertices.end(), vec.at(b));
			if (!back) {
				if (is_near(vec.at(b).slope(vec.at(i)), slope)) {
					if (vec.at(i).distance(a) > vec.at(b).distance(a))
						b = i;
